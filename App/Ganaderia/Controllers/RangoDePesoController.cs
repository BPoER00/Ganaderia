...
namespace Ganaderia.Controllers
{
    ...
    {
        private RangoDePesoActions db;

        public RangoDePesoController(ConexionContext _db)
        {
            this.db = new RangoDePesoActions(_db);
        }
        ...
        [HttpPost]
        public async Task<Reply> Post(RANGO_DE_PESO rango_de_peso)
        {
            try
            {
                ...
                var responseAction = await this.db.Guardar(rango_de_peso);
        ...
        [HttpPut("{id}")]
        public async Task<Reply> Put(int? id, RANGO_DE_PESO rango_de_peso)
        {
            try
            {
                if(!ModelState.IsValid)
                ...
                var responseAction = await this.db.Actualizar(id, rango_de_peso);
