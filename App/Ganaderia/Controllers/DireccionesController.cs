...
namespace Ganaderia.Controllers
{
    ...
    {
        private DireccionesActions db;

        public DireccionesController(ConexionContext _db)
        {
            this.db = new DireccionesActions(_db);
        }
        ...
        public async Task<Reply> Post(DIRECCION direccion)
        {
            try
            {
                ...
                var responseAction = await this.db.Guardar(direccion);
        ...
        [HttpPut("{id}")]
        public async Task<Reply> Put(int? id, DIRECCION direccion)
        {
            try
            {
                if(!ModelState.IsValid)
                ...
                var responseAction = await this.db.Actualizar(id, direccion);
