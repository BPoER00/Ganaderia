...
namespace Ganaderia.Controllers
{
    ...
    {
        private GenerosActions db;

        public GenerosController(ConexionContext _db)
        {
            this.db = new GenerosActions(_db);
        }
        ...
        public async Task<Reply> Post(GENERO genero)
        {
            try
            {
                ...
                var responseAction = await this.db.Guardar(genero);
        ...
        [HttpPut("{id}")]
        public async Task<Reply> Put(int? id, GENERO genero)
        {
            try
            {
                if(!ModelState.IsValid)
                ...
                var responseAction = await this.db.Actualizar(id, genero);
